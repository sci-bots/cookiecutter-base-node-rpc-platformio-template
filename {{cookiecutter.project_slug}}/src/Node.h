...
#include "{{cookiecutter.project_camelcase}}/state_pb.h"
...
class Node;
const char HARDWARE_VERSION_[] = "{{cookiecutter.hardware_version}}";

typedef nanopb::EepromMessage<{{cookiecutter.project_module_name}}_Config,
                              config_validate::Validator<Node> > config_t;
typedef nanopb::Message<{{cookiecutter.project_module_name}}_State,
...
public:
  ...
  static const uint32_t BUFFER_SIZE = {{ cookiecutter.hardware_buffer_size }};  // >= longest property string
  ...
  Node() : BaseNode(),
           BaseNodeConfig<config_t>({{cookiecutter.project_module_name}}_Config_fields),
           BaseNodeState<state_t>({{cookiecutter.project_module_name}}_State_fields) {
